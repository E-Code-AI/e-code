...
- **Database Backup:** Use `tsx scripts/backup-database.ts` for backup (creates timestamped SQL file in `backups/`), `tsx scripts/backup-database.ts --restore <file>` for restore. Supports pg_dump when available, falls back to postgres driver. Cloud storage upload with `--cloud` flag (requires GCS_BACKUP_BUCKET).
