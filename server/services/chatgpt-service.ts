  ...
  async createSession(userId: string, projectId?: string): Promise<ChatSession> {
    ...
    const session: ChatSession = {
      ...
      messages: [
        {
          role: 'system',
          content: `You are ChatGPT with GPT-5.2, an AI assistant integrated into the E-Code Platform. 
...
Current model: GPT-5.2 (Dec 2025 flagship with advanced reasoning)`
        }
      ],
      createdAt: new Date(),
      updatedAt: new Date(),
    ...
    try {
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert programmer using GPT-5.2 with enhanced coding capabilities. Generate clean, efficient, and well-documented code.'
          },
          {
            role: 'user',
            content: codePrompt
