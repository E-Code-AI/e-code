...
InsertNewsletterDelivery,
InsertNewsletterSubscriber,
InsertPaneConfiguration,
InsertProject,
InsertProjectAiRule,
InsertProjectAuthConfig,
InsertProjectAuthUser,
InsertPromptTemplate,
InsertPromptTemplateRating,
InsertPromptUsageHistory,
InsertResourceMetric,
InsertScreenshot,
InsertSecurityScan,
InsertSecurityScanSettings,
InsertSubmission,
InsertTaskSummary,
InsertTemplate,
InsertTerminalLog,
...
InsertTimeTracking,
InsertUser,
InsertVoiceVideoParticipant,
InsertVoiceVideoSession,
InsertVulnerability,
LspDiagnostic,
MentorProfile,
MobileSession,
...
insertAlertSchema,
insertAutoscaleDeploymentSchema,
insertBudgetLimitSchema,
insertCustomDomainSchema,
insertCustomerRequestSchema,
insertDynamicIntelligenceSchema,
insertGitCommitSchema,
insertGitRepositorySchema,
insertKeyValueStoreSchema,
insertNotificationPreferenceSchema,
insertNotificationSchema,
insertObjectStorageBucketSchema,insertObjectStorageFileSchema,
insertReservedVmDeploymentSchema,
insertScheduledDeploymentSchema,insertStaticDeploymentSchema,
insertUsageAlertSchema,
insertUserCreditsSchema,
insertWebSearchHistorySchema,
keyValueStore,
lspDiagnostics,
mentorProfiles,
mobileSessions,
newsletterCampaigns,newsletterDeliveries,
newsletterSubscribers,
normalizeUserId,
notificationPreferences,
objectStorageBuckets,objectStorageFiles,
paneConfigurations,
passwordResetTokens,
...
function _num(val: string | number | undefined | null): number {
...
import { SQL,and,desc,eq,gte,ilike,inArray,isNull,lt,lte,or,sql } from "drizzle-orm";
...
export interface IStorage {
  ...
  createFile(data: {projectId: string | number; path: string; content: string }): Promise<File>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUser(id: string | number): Promise<User | undefined> {
    const [user] = await this.db.select().from(users).where(eq(users.id, _num(id)));
    return user;
  }
  ...
  async getProjectsByUser(userId: string | number): Promise<Project[]> {
    return await this.db.select().from(projects).where(eq(projects.ownerId, _num(userId)));
  }
  ...
  async getProjectsByUserIdPaginated(userId: string | number, limit: number, offset: number): Promise<{ projects: Project[]; total: number }> {
    const [projectsList, countResult] = await Promise.all([
      this.db
        .select()
        .from(projects)
        .where(eq(projects.ownerId, _num(userId)))
  ...
  async getProjectBySlug(slug: string, ownerId?: string | number): Promise<Project | null> {
    try {
      const condition =
        ownerId !== undefined
          ? and(eq(projects.slug, slug), eq(projects.ownerId, _num(ownerId)))
  ...
  async getFilesByProjectId(projectId: string | number): Promise<File[]> {
    return await this.db.select().from(files).where(eq(files.projectId, _num(projectId))).orderBy(files.path);
  }
  ...
  async getFileByPath(projectId: string | number, path: string): Promise<File | undefined> {
    const [file] = await this.db
      .select()
      .from(files)
      .where(and(eq(files.projectId, _num(projectId)), eq(files.path, path)))
      .limit(1);
    return file;
  }

  async createFile(data: { projectId: string | number; path: string; content: string }): Promise<File>;
  async createFile(fileData: InsertFile): Promise<File>;
  async createFile(
    fileData: InsertFile | { projectId: string | number; path: string; content: string }
  ): Promise<File> {
