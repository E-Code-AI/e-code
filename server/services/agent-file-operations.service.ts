...
export class AgentFileOperationsService extends EventEmitter {
  ...
  private readonly ALLOWED_SPECIAL_FILES = [
    ...
    '.eslintignore', '.prettierignore', '.dockerignore', '.npmignore', '.stylelintignore', '.keep', '.gitkeep',
  ...
  async createOrUpdateFile(
    ...
    try {
      ...
      // ✅ FIX (Apr 21 2026): For the root-level index.ts/index.js of any generated
      // project that contains a frontend/ (or root) package.json with vite, replace
      // content with a launcher. Works even if the agent generated an empty index.ts.
      if (filePath === 'index.ts' || filePath === 'index.js') {
        try {
          const ctxP = session.context as { projectId?: number } | undefined;
          const pid = ctxP?.projectId;
          if (pid) {
            const candidates = ['package.json', 'frontend/package.json', 'client/package.json'];
            let viteDir: string | null = null;
            let viteWorkingDir: string = '.';
            for (const candidatePath of candidates) {
              const rows = await db.select().from(files).where(and(
                eq(files.projectId, pid),
                eq(files.path, candidatePath)
              )).limit(1);
              const r = rows[0];
              if (r && r.content) {
                try {
                  ...
                  if (deps.vite && pkg.scripts && pkg.scripts.dev) {
                    viteDir = candidatePath.includes('/') ? candidatePath.replace('/package.json','') : '.';
                    viteWorkingDir = viteDir;
                    break;
                  }
                } catch {}
              }
            }
            if (viteDir && !(content || '').includes('[launcher]')) {
              logger.info(`[FileOps] Overriding ${filePath} with Vite launcher for project ${pid} (viteDir=${viteDir})`);
              content = [
                ...
                "const rootDir = dirname(__file);",
                "const workDir = " + JSON.stringify(viteWorkingDir) + " === '.' ? rootDir : join(rootDir, " + JSON.stringify(viteWorkingDir) + ");",
                "",
                "if (!existsSync(join(workDir, 'node_modules'))) {",
                "  console.log('[launcher] Installing dependencies in ' + workDir + ' (runs once, may take 60-90s)...');",
                "  const r = spawnSync('npm', ['install','--prefer-offline','--no-audit','--progress=false','--loglevel=error'], { stdio: 'inherit', cwd: workDir });",
                "  if (r.status !== 0) { console.error('[launcher] npm install failed with code ' + r.status); process.exit(1); }",
                "  console.log('[launcher] Install complete');",
                "}",
                "",
                "const port = process.env.VITE_PREVIEW_PORT || process.env.PORT || '5174';",
                "console.log('[launcher] Starting vite dev on port ' + port + ' (cwd=' + workDir + ')');",
                "const vite = spawn('npx', ['vite','--host','0.0.0.0','--port', port], { stdio: 'inherit', cwd: workDir });",
