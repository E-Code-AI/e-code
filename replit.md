...
- **Database Auto-Provisioning:** Databases provision asynchronously (fire-and-forget) on project creation to avoid API gateway timeouts. Neon/K8s providers take 45-60s so sync await is forbidden. Frontend polls status via `/api/database/project/:id`. Multi-provider fallback: if Neon fails, auto-falls back to local PostgreSQL provider. Frontend auto-retries: if user opens DatabasePanel and database has status='error', triggers auto-retry; if no database exists, triggers auto-provision. Files: `server/routes/projects.router.ts`, `client/src/components/ide/DatabasePanel.tsx`, `server/services/project-database-provisioning.service.ts`
- **Agent Bootstrap Resilience:** Agent panel has 15-second timeout fallback to prevent infinite "Initializing Agent" loading state. If `POST /api/agent/conversation` fails or times out, UI gracefully exits loading state via `bootstrapFailed` flag. Users can still chat even if initial conversation setup fails. Files: `client/src/components/ai/ReplitAgentPanelV3.tsx`
