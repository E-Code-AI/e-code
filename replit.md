...
- Users Router Route Ordering (Critical): In `server/routes/users.router.ts`, all FIXED-path GET routes (`/usage`, `/billing`, `/billing-summary`, `/ai-preferences`, `/:id/subscription`, `/:id/usage`) MUST be registered BEFORE the wildcard `GET /:id` route. Otherwise Express captures them as user lookups. The current order: me → search → username/:username → usage → billing → billing-summary → ai-preferences → /id/subscription → /id/usage → /id.
- AI Preferences Endpoint: `GET /PUT /api/users/ai-preferences` — in-memory per-user store (resets on server restart) for `agentMemoryEnabled` and `trainingOptOut`. Wired to the AI & Privacy section of `/settings`.
