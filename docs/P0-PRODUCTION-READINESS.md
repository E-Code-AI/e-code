...
| 2 | États mémoire critiques | Partiellement corrigé sur les surfaces P0 citées | Idempotence et agent session cache n'utilisent plus le fallback mémoire en production; scheduler stocke les tâches en cours dans Redis; collaboration persiste/publie la présence via Redis |
...
`npm run prod:p0` échoue actuellement sur `database` et `stripe`. `npm run db:audit` doit être relancé après correction de `DATABASE_URL`.
