...
- Admin AI Studio (`/admin/chatgpt`): Rebuilt with 3 tabs — AI Chat (streaming SSE via `POST /api/admin/chatgpt/stream`), Client Projects (browse/edit all users' files), Live Sessions (monitor+terminate). Supports 6 models: OpenAI gpt-5.1, gpt-4.1, o3, o4-mini + Anthropic claude-opus-4-20250514, claude-sonnet-4-20250514. Service in `server/services/chatgpt-service.ts`, router in `server/routes/chatgpt.router.ts`. Endpoints: `GET /api/admin/chatgpt/models`, `POST /api/admin/chatgpt/stream`, `GET /api/admin/chatgpt/all-projects`, files CRUD.
- rate_limit_violations table: Created in DB (March 2026) to match `shared/schema.ts`. Used by `/api/admin/monitoring/rate-limit-stats` and `/api/admin/monitoring/rate-limit-violations`.
...
- AI Model Defaults (March 2026, confirmed working): OpenAI default=`gpt-5.1`, fast=`gpt-5-nano`; Anthropic default=`claude-sonnet-4-20250514`, fast=`claude-3-5-haiku-20241022`, quality=`claude-opus-4-20250514`; Gemini default=`gemini-2.5-flash`, quality=`gemini-2.5-pro`; xAI default=`grok-3`, fast=`grok-3-mini`. All hardcoded stale model IDs (gpt-4o, gpt-4, claude-3-5-sonnet-20241022, claude-3-opus-20240229) replaced across 35+ files. Model normalizer (`server/utils/model-normalizer.ts`) maps all aliases/old IDs to confirmed-working real IDs. Fake future IDs (claude-sonnet-4-5-20250929 etc.) purged. gpt-5.x models require `max_completion_tokens` not `max_tokens`, and do NOT support `temperature`.
